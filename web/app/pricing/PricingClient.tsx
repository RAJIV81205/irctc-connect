...
export default function PricingClient({
  initialPlans,
  initialOfferEndsAt,
  initialPlanContactEmail,
}: {
  initialPlans: PricingPlan[];
  initialOfferEndsAt: string | null;
  initialPlanContactEmail: string;
}) {
  return (
    <Suspense fallback={<PricingPageFallback />}>
      <PricingPageContent
        ...
        initialPlanContactEmail={initialPlanContactEmail}
      />
    </Suspense>
  );
}
...
function PricingPageContent({
  ...
  const pricingPlans = initialPlans;
  const isLoadingPlans = false;
  const offerEndsAt = initialOfferEndsAt;
  const planContactEmail = initialPlanContactEmail;
