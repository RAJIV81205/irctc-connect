...
import { Copy, Key, Mail, User as UserIcon, Check, Zap, Activity, Clock } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const usagePercentage = dbUser.limit > 0 ? (dbUser.usage / dbUser.limit) * 100 : 0;

  return (
    <main className="min-h-screen pt-28 pb-16 bg-[radial-gradient(circle_at_10%_0%,#0f172a_0%,transparent_40%),radial-gradient(circle_at_90%_100%,#082f49_0%,transparent_35%),linear-gradient(160deg,#020617,#0f172a,#0f172a)] px-6 text-slate-100">
      <Header />
      <div className="mx-auto max-w-4xl space-y-8">
        
        {/* Header */}
        <div>
          <h1 className="text-4xl font-bold tracking-tight text-white drop-shadow-sm">Dashboard</h1>
          <p className="mt-2 text-slate-400">Manage your API keys and usage</p>
        </div>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          ...
          <div className="group relative overflow-hidden rounded-2xl border border-slate-700/60 bg-slate-900/40 p-6 backdrop-blur-xl transition hover:border-slate-600/80">
            ...
            <h2 className="mb-6 text-lg font-semibold text-white/90">Profile Details</h2>
            <div className="space-y-4 relative z-10">
              <div className="flex items-center gap-4">
                ...
                <div className="min-w-0">
                  ...
                  <p className="truncate font-medium text-slate-100">{dbUser.name}</p>
                </div>
              </div>
              <div className="flex items-center gap-4">
                ...
                <div className="min-w-0">
                  ...
                  <p className="truncate font-medium text-slate-100">{dbUser.email}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="group relative overflow-hidden rounded-2xl border border-slate-700/60 bg-slate-900/40 p-6 backdrop-blur-xl transition hover:border-slate-600/80">
            ...
            <div className="space-y-6 relative z-10">
              <div className="flex items-end justify-between">
                <div>
                  <p className="text-4xl font-light text-white tracking-tight">{usageLeft} <span className="text-xl font-normal text-slate-500">left</span></p>
                  <p className="mt-1 text-sm text-slate-400">from {dbUser.limit} total requests</p>
                </div>
              ...
              <div className="relative h-2 w-full overflow-hidden rounded-full bg-slate-800/80 ring-1 ring-inset ring-white/5">
                <div 
                  className={`absolute left-0 top-0 h-full rounded-full transition-all duration-1000 ${
                    ...
                    usagePercentage > 75 ? 'bg-amber-400 shadow-[0_0_10px_rgba(251,191,36,0.5)]' : 
        ...
        <div className="relative overflow-hidden rounded-2xl border border-slate-700/60 bg-slate-900/40 p-6 backdrop-blur-xl transition hover:border-slate-600/80">
          ...
          <div className="mb-4 flex items-center gap-3 relative z-10">
            ...
            <h2 className="text-lg font-semibold text-white/90">Your Secret API Key</h2>
          </div>
          <p className="mb-6 text-sm text-slate-400 max-w-2xl relative z-10">
            Include this key in your requests to authenticate with the API. Keep it safe and do not share it publicly.
          </p>
